...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private static final String logPathDir = isSdcardExist() ? (Environment
            .getExternalStorageDirectory()
            + File.separator + "com_ijustjump_loop" + File.separator) : null;
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        LoggerUtil.f("error_application_exit", ex);
...
//        }.start();
    ...
    private String saveCrashInfo2File(Throwable ex) {
        if (!isSdcardExist()){
            Log.e(TAG, "sdcard is note exist");
            return null;
        }
...
//                String path = "/sdcard/com_wstv_cloud_games/";
                ...
                FileOutputStream fos = new FileOutputStream(logPathDir + fileName);
     ...
     * @return 返回文件名称, 便于将文件传送到服务器
     */
    private String saveCrashInfoTo1File(Throwable ex) {
